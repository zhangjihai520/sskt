...
.wxchat-container::-webkit-scrollbar {
...
<template>
  <div
    ...
    :style="{ backgroundColor: wrapBg, overflow: hidden }"
  >
    <div
      class="window"
    ...
    >
      ...
      <div class="loading" v-if="dataArray && dataArray.length == 0">
        <div style="margin-top: 280px; text-align: center; font-size: 16px">
          <span>未查找到聊天记录</span>
        </div>
      </div>
        ...
        <div id="message" class="message" :style="{ overflow: hidden }">
          <ul class="scroll_ul">
            <li
              ...
              :key="message.TalkMessageId"
              :class="
                message.UserId == ownerUserId ? 'an-move-right' : 'an-move-left'
              "
            >
              <p class="time">
                <span v-text="process(message.ShowDateTime)"></span>
              </p>
              <!--<p class="time system" v-if="message.type==10000"> <span v-html="message.content"></span> </p>-->
              <div
                :class="'main' + (message.UserId == ownerUserId ? ' self' : '')"
              >
                <img
                  class="avatar"
                  width="40"
                  height="40"
                  :src="message.UserFace"
                />
                <div>
                  <div class="username">
                    <span v-text="message.UserTrueName"></span>
                  </div>
                  <!-- 文本 -->
                  <div
                    :class="message.ToUserId > 0 ? 'privateText' : 'text'"
                    v-emotion="
                      message.ToUserId > 0
                        ? '私信：' + message.Content
                        : message.Content
                    "
                    v-if="message.TalkTypeId == 1"
                  ></div>

                  <!-- 图片 -->
                  <div
                    :class="message.ToUserId > 0 ? 'privateText' : 'text'"
                    v-else-if="message.TalkTypeId == 2"
                  >
                    <!--<img :src="message.ThumbImageUrl" class="image" alt="聊天图片">-->
                    <img
                      :src="message.ThumbImageUrl"
                      @click="seeBigTu(message.AttachmentUrl)"
                    />
                  </div>
        ...
        this.getUpperData().then(function (result) {
          if (!result.TalkMessageList || result.TalkMessageList.length == 0) {
            ...
            var scrollContainer = document.getElementById(
              "scrollLoader-container"
            );
            var scrollContent = document.getElementById("scrollContent");
            if (me.isIos) {
              scrollContainer.scrollTop =
                scrollContainer.scrollHeight - me.beforeHight;
              me.beforeHight = scrollContainer.scrollHeight;
            } else {
              scrollContainer.scrollTop =
                scrollContainer.scrollHeight - scrollContent.offsetHeight;
              var wxchatScrollContainer = document.getElementById(
                "wxchat-container"
              );
              wxchatScrollContainer.scrollTop =
                wxchatScrollContainer.scrollHeight;
            }
          }
          me.isUpperLaoding = false;
        });
