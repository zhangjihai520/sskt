  ...
  <div class="page">
    ...
    <div class="send" gutter="0">
      <div v-if="UserStatus == 0" class="txt">
        <div v-if="UserId != HelpTeacherId" class="xw-chat-tool-item">
          <check-icon class="xw-checkbox" :value.sync="cbCheck"
            ><span :class="cbCheck ? 'private_red' : 'private_gray'"
              >师</span
            ></check-icon
          >
        </div>
        <x-textarea
          :class="UserId == HelpTeacherId ? 'helpTeacherMsg' : 'msg'"
      ...
      <div v-if="UserStatus > 0" class="noTalk">禁言中</div>
  ...
  methods: {
    ...
    async getLessonData() {
      ...
      let url =
        ...
        current.Uid +
        "_H5";
      (this.UserId = current.Uid), (this.wsUrl = url), this.getMessage();
      this.initWebSocket();
    },
    ...
    websocketonmessage(evt) {
      ...
      if (obj.TalkMessage) {
        if (obj.MessageType != 1) {
          if (
            obj.TalkMessage.TalkTypeId == 1 &&
            obj.TalkMessage.Content.indexOf("**") == -1
          ) {
            ...
            if (obj.TalkMessage.ToUserId == 0 && this.playerReady) {
              var startTime =
                new Date().getTime() - this.playTimeZone - this.devTimeZone;
              ...
              this.player.danmu.sendComment({
                ...
                txt:
                  obj.TalkMessage.UserTrueName + ":" + obj.TalkMessage.Content, //+":"+this.playTimeZone+":"+new Date().getTime()+":"+startTime+":"+ this.devTimeZone, //弹幕文字内容
                style: {
                  color: self.getColor(),
                  fontSize: "16px",
                  padding: "3px 6px",
          ...
          if (
            obj.TalkMessage.ToUserId == 0 ||
            (obj.TalkMessage.ToUserId > 0 &&
              (obj.TalkMessage.ToUserId == this.UserId ||
                obj.TalkMessage.UserId == this.UserId))
          ) {
            let socketData = [];
