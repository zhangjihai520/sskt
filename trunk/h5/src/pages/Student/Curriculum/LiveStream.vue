<template>
  <div class="page">
    ...
    <tab class="tabWrapper">
          <tab-item selected @on-item-click="switchTab(1)">互动区</tab-item>
          <tab-item @on-item-click="switchTab(2)">随堂练习</tab-item>
    </tab>
      <wxChat
        :data="wxChatData"
        ...
        :isShow="activeTab==1"
      ></wxChat>
      <div v-show="activeTab==1" class="send" gutter="0">
        <div v-if="UserStatus == 0" class="txt">
          <!-- <div class="xw-chat-tool-item">
              <check-icon class="xw-checkbox" :value.sync="cbCheck"><span :class="cbCheck?'private_red':'private_gray'">师</span></check-icon>
          </div> -->
          <x-input
            class="msg"
            ...
            @blur="scrollToBottom"
          ></x-input>
          <div class="xw-chat-tool-item"  style="margin-left:60px">
          ...
          <div class="xw-chat-tool-item send">
            <transition name="send">
              <a
                ...
                @click="sendMessage"
              >发送</a>
              <!-- <photograph @upload="upload ">v-if="content.trim().length && UserStatus==0"</photograph> -->
            </transition>
          </div>
        </div>
      ...
      <div style="overflow-y: auto;" v-show="activeTab==2">
        <div v-if="ExamSetId">
          <div v-if="ExamSetStatus > 0">
            <div v-if="!IsFinish">
              <classwork :examSetId="ExamSetId" @changeClassWorkFinish = "changeClassWorkFinish"/>
            </div>
            <div style="overflow-y: auto;" v-else>
              <classworkEvaluationResults :examSetId="ExamSetId" />
            </div>
          </div>
          <div style="margin-top: 200px;text-align:center; font-size: 16px; color: #b0b0b0;" v-else>
            <span>随堂作业待开启</span>
          </div>
        </div>
        <div style="margin-top: 200px;text-align:center; font-size: 16px; color: #b0b0b0;" v-else>
...
import classworkEvaluationResults from "../../../components/classworkEvaluationResults.vue";
import classwork from "../../../components/classwork.vue";
...
export default {
  ...
  methods: {
    ...
    async getMessage() {
      ...
      this.ExamSetStatus=result.ExamSetStatus
    ...
    async getLessonData() {
      ...
      this.ExamSetId = current.ExamSetId;
      this.IsFinish = current.IsFinish;
      let result = [];
      if (current.VideoUrl) {
    ...
    scrollToBottom: function () {
                this.$nextTick(() => {
                    ...
                    if(messageContainer.offsetHeight < scrollContainer.offsetHeight)
                    {
