          ...
          <div slot="content" class="card-demo-flex">
            ...
            >
              <x-button
                ...
                @click.native="signUp($event,i.SubjectId, 2)"
                :data-id="i.SubjectId"
                >
                {{
                  btn(i.StatusFlag, i.IsRegister, i.RegistEndTime)
                }}</x-button
              >
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    //点击报名课程
    async registSubject(subjectId, type) {
      var _this = this;
      let param = {
        SubjectId: subjectId
      };
      await registSubject(param).then(res => {
        if (type == 1) {
          let list = _this.dateList;
          for (let val of list) {
            if (_this.selectedDate == val.BeginDate) {
              for (let a of val.SubjectList) {
                if (subjectId == a.SubjectId) {
                  a.IsRegister = true; //已报名
                  break;
                }
              }
            }
          ...
          _this.searchedList = list;
          _this.$forceUpdate(); //重新渲染组件
        }
        if (res == 1) {
          _this.msg = "报名成功";
        ...
        this.getCurriculumList(_this.currentViewMonth);
        _this.show = true;
      });
    },
    ...
    signUp(e,subjectId, type) {
      // 只有 报名 能点击
      if (e.target.innerText == "报名")
      {
        this.registSubject(e.srcElement.dataset.id, type);
      }else if(e.target.innerText == "进入课堂")
      {
        const url = `/Online/Student/LiveStream?subjectId=${subjectId}`;
        this.$router.push(url)
      }else if(e.target.innerText == "回看")
      {
        const url = `/Online/Student/RecordStream?subjectId=${subjectId}`;
        this.$router.push(url)
      }else{
        return;
      }
    ...
    // 按钮置灰状态
    btnState(StatusFlag, IsRegister, IsMaxRegisterNumber, RegistEndTime) {
      if (IsRegister) {
        if (StatusFlag == 5) {
          return false;
    ...
    // 按钮文字状态
    btn(StatusFlag, IsRegister, RegistEndTime) {
      let name = "";
      if (IsRegister) {
        if (StatusFlag == 6) {
          name = '进入课堂';
  ...
  .list {
    ...
    .itemWrap {
      .vux-card-content {
        ...
        box-shadow:2px 2px 10px #0905e7;
      }
      .item {
