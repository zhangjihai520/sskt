    ...
    const columns = [
      {
        ...
        render: (name, record) => {
          return (
            <span>
              {(!record.IsLecturer && record.StatusFlag != 6) ? (<img
                className={style.img}
                alt=""
              ...
              ></img>) :(<img
                ...
                src={record.ImagePath ? record.ImagePath : `/images/common/img_${shortCodeFilter(record.ShortCode)}.jpg`}
                onClick={() => {
                  window.open(`${TeaBasePath}/Curriculum/ClassManagement?SubjectId=${record.SubjectId}`, '_blank');
                }}
              ></img>) }
        ...
        render: (
          ...
          record //record.StatusFlag === 6 上课中，也可以操作上课按钮
            ...
            <Fragment>
              ...
              {record.IsLecturer && record.SubjectGenreId < 3 && record.StatusFlag != 6 && (
