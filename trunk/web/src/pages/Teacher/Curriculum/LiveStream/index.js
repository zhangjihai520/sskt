...
import { Button, Checkbox, Input, message, Tabs, Popover, Collapse, Row, Col, Badge, Modal, Spin, Empty } from 'antd';
import $ from 'jquery';
...
export default class ListenStream extends Component {
    ...
    this.props
      ...
      .then(result => {
        ...
        this.setState(
          {
            UserId: result.Uid,
            subjectRoomId: Number(result.Channel),
            wsUrl: url
  ...
  changeUserStatus = (userId, status) => {
    this.props
      .dispatch({
        type: 'stuModel/ChangeUserStatus',
        payload: {
          SubjectId: this.SubjectId,
          ToUserId: userId,
          Status: status
  ...
  noTalkingChange = e => {
    this.props
      .dispatch({
        type: 'stuModel/ChangeRoomStatus',
        payload: {
          SubjectId: this.SubjectId,
...
handleGoStatistics = () => {
  ...
  render() {
    ...
    const memberOper = item => {
      if (this.state.RoomUserType != 2 && item.RoomUserType != 1 && item.RoomUserType != 3) {
        switch (item.Status) {
          case 0:
            return (
              <div>
    ...
    const classWorkContent = () =>{
      if(data.ExamSetId)
      {
        if(ExamSetStatus>0)
        {
          ...
          return (
            <div className={style.emptyContent}>
                <Button onClick={() => {this.ChangeExamSetStatus(1);}}>开启随堂练习</Button>
            </div>
          )
        }
      }else{
        return (
          <div className={style.emptyContent}>
            <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂未布置随堂作业" />
          </div>
        )
      }
    }
    ...
    return (
      <div className={style.contentArea}>
        ...
        <Row className={style.videoPanel}>
          {data.ExamSetId && ExamSetStatus>0 &&(
            ...
            <Popover className={style.triggerDesc} placement="rightTop" visible={this.state.cardVisible} content={answerSheet()} onVisibleChange={this.handleCardVisibleChange} trigger="click"> 
              ...
              <div style={{marginTop:'10px'}}  onClick={() => {this.handleGoStatistics();}}>
                <img src={statisticsIcon} title="统计" />
              </div>
            </div>
          )}
          ...
          <Col flex="auto" className={style.video_center}>
            ...
            <div className={style.inputWrap}>
                  ...
                  <div className={style.newAffixUploadWrap}>
                    ...
                    <span className={style.fileInput} style={{ marginLeft: '10px' }}>
                    ...
                    <Checkbox style={{marginLeft: '10px', color: '#989b9b' }} checked={this.state.noTalkingChecked} onChange={this.noTalkingChange}>
                      全体禁言
                    </Checkbox>
          ...
          <Col flex="200px" className={style.video_right}>
                  <Collapse defaultActiveKey={['1']} style={{ width: '200px', height: '699px', overflowY: 'auto' }}>
                    <Panel key="1" header={
                      <span>
