...
import { Layout, Menu, Icon, Button, Checkbox, Input, message, Tabs, Popover, Collapse, Row, Col, Badge, Modal, Spin, Empty } from 'antd';
import $ from 'jquery';
...
import statisticsIcon from 'assets/common/statistics.png'
...
export default class ClassManagement extends Component {
  ...
  connectWebsocket = () => {
    ...
    // let url = protocol + "://192.168.50.17:8080/api/chat/" + this.state.seleKey + "/" + this.state.userId + "_web";
    let url = protocol + "://sskt.nceduc.cn/api/chat/" + this.state.seleKey + "/" + this.state.userId + "_web";
    this.setState(
      {
        UserId: this.state.userId,
        subjectRoomId: Number(this.state.seleKey),
        wsUrl: url
      }, () => {
        this.getMessage(0);
        this.initWebSocket();
  ...
  getSource = () => {
    ...
    this.props
      .dispatch({
        type: 'curriculumModel/GetSubjectVideo',
        payload: {
          SubjectId: SubjectId
        }
      })
  ...
  getMessage = (toUserId) => {
    ...
    this.props
      .dispatch({
        type: 'curriculumModel/GetMessageBySubjectRoomId',
        payload: {
          SubjectId: this.SubjectId,
          BoundaryId: BoundaryId,
          SubjectRoomId: this.state.seleKey,
  ...
  upload = (file, type) => {
    ...
    formData.append('SubjectRoomId',  this.state.allSend ? 0 : Number(this.state.seleKey));
    this.props
      .dispatch({
        type: 'curriculumModel/SendFile',
        payload: formData
      });
  ...
  handleSubmitMessage = () => {
    ...
    // console.log(this.state.allSend)
    this.props
      .dispatch({
        type: 'curriculumModel/SendMessage',
        payload: {
          SubjectId: this.SubjectId,
          SubjectRoomId: this.state.allSend ? 0 : Number(this.state.seleKey),
  ...
  changeUserStatus = (userId, status) => {
    this.props
      .dispatch({
        type: 'curriculumModel/ChangeUserStatus',
        payload: {
          SubjectId: this.SubjectId,
          ToUserId: userId,
          Status: status,
  ...
  removeMessage = (messageId) => {
    this.props
      .dispatch({
        type: 'curriculumModel/RemoveMessage',
        payload: {
          SubjectRoomId: this.state.subjectRoomId,
          TalkMessageId: messageId
        }
      });
  };
  //改变值
  ...
  noTalkingChange = e => {
    ...
    if (e.target.id == 'currentRoom') {
      this.props
        .dispatch({
          type: 'curriculumModel/ChangeRoomStatus',
          payload: {
            SubjectId: this.SubjectId,
        ...
        .then(result => {
          if (result == 1) {
            this.setState({
              noTalkingChecked: e.target.checked,
              panes: initialPanes,
              userInfoList: [],
            }, () => {
              ws.close();
  ...
  ChangeExamSetStatus = (status) => {
    this.props
      .dispatch({
        type: 'stuModel/ChangeExamSetStatus',
        payload: {
          SubjectId: this.SubjectId,
          Status: status
        }
  ...
  handleGoStatistics = () => {
    ...
    if (data.ExamSetId) {
      window.open(`${TeaBasePath}/Curriculum/Work/Statistics?ExamSetId=${data.ExamSetId}&SubjectId=${this.SubjectId}`, '_blank');
    } else {
      message.destroy();
  ...
  render() {
    const { data, message, tarVideoStream, noTalkingChecked, userInfoList, emojiData, panes, activeKey, unReadKey, topicListData = {}, answerList = [], serialNumber, ExamSetStatus } = this.state;
    ...
    const messageOper = item => {
      return (<div>
        <a href="javascript:void(0);" onClick={() => this.removeMessage(item.TalkMessageId)}>删除</a>
      </div>
      );
    };
    const memberOper = item => {
      switch (item.Status) {
        case 0:
          return (
            <div>
              <a href="javascript:void(0);" onClick={() => this.changeUserStatus(item.UserId, 1)}>禁言</a>&nbsp;&nbsp;&nbsp;&nbsp;
              <a href="javascript:void(0);" onClick={() => this.changeUserStatus(item.UserId, 2)}>加入黑名单</a>
            </div>
          );
          break;
        case 1:
          return (
            <div>
              <a href="javascript:void(0);" onClick={() => this.changeUserStatus(item.UserId, 0)}>解除禁言</a>&nbsp;&nbsp;&nbsp;&nbsp;
              <a href="javascript:void(0);" onClick={() => this.changeUserStatus(item.UserId, 2)}>加入黑名单</a>
            </div>
          );
          break;
        case 2:
          return (
            <div className={style.blackUser}>
              <a href="javascript:void(0);" onClick={() => this.changeUserStatus(item.UserId, noTalkingChecked ? 1 : 0)}>解除黑名单</a>
            </div>
          );
          break;
      }
    ...
    return (
      <Layout>
        <Sider width={150} className={style.slider}>
          <div className={style.sliderTop} >
            所有房间
          </div>
          {data.HelperIds && data.HelperIds.length > 0 && (
            <Menu theme="dark" mode="inline" defaultSelectedKeys={[this.state.seleKey.toString()]}>
              {data.HelperIds && data.HelperIds.map((a, i) => (
                <Menu.Item key={a.toString()} onClick={this.changeClass}>
                  <Icon type="pie-chart" />
                  <span>{i + 1 + '号房间'}</span>
                </Menu.Item>
              ))}
        ...
        <Layout>
          <Header className={style.header}>
            <Row className={style.contentTop}>
              <span style={{color:'#fff'}}>课程名称：{data.SubjectName}</span>

              <Checkbox  style={{ marginLeft: '100px', color: '#fff' }} checked={this.state.allchanged} onChange={this.noTalkingChange} id='allRoom' >
                禁言所有房间
              </Checkbox>

              <Checkbox style={{ marginLeft: '100px', color: '#fff' }} checked={this.state.allSend} onChange={this.changeAllsend} id='allSend' >
                群发信息
              </Checkbox>
            </Row>
          </Header>
              ...
              <Row className={style.videoPanel}>
                {data.VideoUrl && data.PPTVideoUrl && data.VideoUrl[0] != null && data.PPTVideoUrl[0] != null && (
                  <div className={style.leftTopPanel}>
                    <div
                      onClick={() => {
                        this.handleSwitch(tarVideoStream);
                      }}
                    >
                ...
                <Col flex="469px" className={style.video_leftPanel} >
                                ...
                                <div key={item.TalkMessageId} className={item.UserId == this.state.UserId ? style.anMoveRight : style.anMoveLeft}>
                                  ...
                                  <div className={style.messageItemWrap} key={`message${index}`}>
                                    ...
                                    <div className={style.item}>
                                      ...
                                      <Popover key={item.UserId} content={messageOper(item)} >
                                        {messContent(item)}
                                      </Popover>
                                    </div>
                                  </div>
                  ...
                  <div className={style.inputWrap}>
                    ...
                    <div className={style.newAffixUploadWrap}>
                      <span className={style.fileInput} >
                        <Popover className={style.triggerDesc} placement="top" visible={this.state.emotionVisible} content={emotion()} onVisibleChange={this.handleVisibleChange} trigger="click">
                          <div key='1'>
                            <img src={imgEmojImg} />
                      ...
                      <span className={style.fileInput}>
                        ...
                        <input
                          style={{width:'80px !important',background:'red'}}
                        ...
                        <Checkbox id='currentRoom' className={style.fileInput} style={{color: '#989b9b', zIndex: '999 ！important',marginLeft:'10px' }} checked={this.state.noTalkingChecked} onChange={this.noTalkingChange}>
                          禁言当前房间
                        </Checkbox>
                      ...
                      <Button className={style.submitBtn} type="primary" onClick={this.handleSubmitMessage}>
                        发送
                      </Button>
                    </div>
                  </div>
                ...
                <Col flex="200px" className={style.video_right}>
                  <Collapse defaultActiveKey={['1']} style={{ width: '200px', maxHeight: '580px', overflowY: 'auto' }}>
                    <Panel key="1" header={
                      <span>
                    ...
                    >
                      {userInfoList &&
                        userInfoList.map((item, index) => {
                          if (item.IsOnline && this.state.UserId != item.UserId && item.RoomUserType != 3) {
                            return (
                              <Popover key={item.UserId} content={memberOper(item)} title={item.UserName} >
                                <span key={item.UserId} className={style.item} onDoubleClick={() => this.add(item)}>
