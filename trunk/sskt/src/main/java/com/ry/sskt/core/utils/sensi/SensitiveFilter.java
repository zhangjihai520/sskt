package com.ry.sskt.core.utils.sensi;
...
public class SensitiveFilter implements Serializable {
    ...
    public void init() {
        try {
            /**ResourcePatternResolver resolver = new PathMatchingResourcePatternResolver();
             Resource resource = resolver.getResource("classpath:sensi_words.txt");
             InputStream is = resource.getInputStream();
             BufferedReader br = new BufferedReader(new InputStreamReader(is, "UTF-8"));**/
            List<SensiWord> words = talkMessageService.loadSensiWord();
            DEFAULT = new SensitiveFilter(words);
        } catch (Exception e) {
            log.error("价值敏感词库加载失败！");
        }
    }
     ...
     * @date 2017年1月5日 下午4:21:06
     */
    public SensitiveFilter(List<SensiWord> words) {
        for (SensiWord word : words) {
            if (null != word && StringUtils.isNotBlank(word.getContent())) {
                put(word.getContent());
            }
        }
    }
