...
public class ExamSetController {
    ...
    @ApiOperation("作答结果统计")
    @ApiImplicitParam(name = "request", dataType = "GetAnswersStatisticsApiRequest", paramType = "body", required = true, value = "作答结果统计入参实体")
    @PostMapping("/GetAnswersStatistics")
    public ResponseBase GetAnswersStatistics(@RequestBody GetAnswersStatisticsApiRequest request) {
        try {
            return examSetActionService.GetAnswersStatistics(request);
        } catch (Exception ex) {
            log.error(String.format("作答结果统计报错，参数：%s", JSON.toJSONString(request)), ex);
            return ResponseBase.GetErrorResponse();
        }
    }
    @ApiOperation("作业上架")
