...
public class SubjectActionService implements ISubjectActionService {
    ...
    private GetSubjectListDetail CreateDetail(SubjectListByFilterView d, List<Course> courseList, List<Grade> gradeList, GetSubjectListApiRequest apiRequest) {
        ...
        var teacherNameList = new ArrayList<String>();
        teacherNameList.add(d.getTeacherNameList().split(",")[0]);
        return new GetSubjectListDetail()
                ...
                .setTeacherNameList(teacherNameList)
    ...
    public ResponseBase GetStudentSubjectList(GetStudentSubjectListApiRequest request) {
        ...
        if (request.getIsMySubject() == 1) {
            ...
            var studentAnswerList = studentAnswerService.getStudentAnswerBySubjectIds(request.getUserId(), StringUtils.join(subjectIdList, ","));
            ...
            if (userSubjectRoom == null) {
                if (subject.getTeacherId() == request.getUserId()) {
                    ...
                    if (CollectionUtils.isNotEmpty(subjectRooms)) {
                        result.setHelperTeacherId(subjectRooms.get(0).getTeacherId());
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public ResponseBase UpdateSubjectStatusOfAny(UpdateSubjectStatusOfAnyApiRequest request) throws Exception {
        int id = 0;
        boolean error = false;
    ...
    /// <returns></returns>
    private int SaveSubject(int id, UpdateSubjectStatusOfAnyApiRequest request, List<VHVideoView> list) throws Exception {
        ...
        //更新缓存
        subject = subjectService.getByKey(id, false);
        if (subject.getClassStateId() == 3) {
            weiKeService.Save(new WeiKe(subject));
        }
        return 1;
    }
