...
public class TalkMessageService implements ITalkMessageService {
    ...
    @Override
    public void processMessage(TalkMessage message) {
    ...
    private void sendMessage(TalkMessage message) {
        ...
        if (cache == null) {
            ...
            if (nowSecond > cache.getLastShowTimeZone() + 300) {
                obj.setLastShowTimeZone(nowSecond);
                message.setShowDateTime(LocalDateTime.now());
            } else {
                obj.setLastShowTimeZone(cache.getLastShowTimeZone());
            }
            obj.setRoomStatus(cache.getRoomStatus()).setExamSetStatus(cache.getExamSetStatus());
        }
        obj.getMessageList().add(message);
        RedisUtil.updateCas(obj);
        talkMessageMapper.insertTalkMessage(message);
    }

    private void removeMessage(TalkMessage message) {
        var obj = new TalkMessageCouchBaseObject(message.getSubjectRoomId());
        var cache = RedisUtil.getObj(obj.getKey(), TalkMessageCouchBaseObject.class);
        if (cache != null) {
            obj.setRoomStatus(cache.getRoomStatus()).setExamSetStatus(cache.getExamSetStatus()).setLastShowTimeZone(cache.getLastShowTimeZone());
            obj.setMessageList(cache.getMessageList().stream().filter(x -> x.getTalkMessageId() != message.getTalkMessageId()).collect(Collectors.toList()));
            RedisUtil.setObj(obj);
        }
        talkMessageMapper.removeTalkMessage(message.getTalkMessageId());
    }
    ...
    public UserSubjectRoomCouchBaseObject getUserSubjectRoomInfo(int userId, int userTypeId, int subjectId) {
        ...
        if (cache != null) {
            RedisUtil.setObj(cache, CommonConst.YEAR_3);
            return cache;
        }
        return null;
    }
    ...
    public void setUserOnline(int userId, int subjectRoomId, boolean online) {
        ...
        RedisUtil.updateCas(obj, CommonConst.YEAR_3);
    }

    @Override
    public void setRoomStatus(int subjectRoomId, int userId, int status) {
        TalkMessageCouchBaseObject cache = getMessageCache(subjectRoomId).setRoomStatus(status);
        RedisUtil.updateCas(cache, CommonConst.YEAR_3);
        ...
        for (User user : allUser) {
            ...
            if (user.getUserId() == userId || user.getRoomUserType() == 3 || user.getRoomUserType() == 1) {
    ...
    @Override
    public void setExamSetStatus(int subjectRoomId, int status) {
        TalkMessageCouchBaseObject cache = getMessageCache(subjectRoomId).setExamSetStatus(status);
        RedisUtil.updateCas(cache, CommonConst.YEAR_3);
    }
    ...
    public List<SensiWord> loadSensiWord() {
        return talkMessageMapper.loadSensiWord();
    }

    @Override
    public void removeTalkMessage(int talkmessageId) {
        talkMessageMapper.removeTalkMessage(talkmessageId);
    }
}
