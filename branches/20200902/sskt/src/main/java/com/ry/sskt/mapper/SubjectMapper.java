...
public interface SubjectMapper {
    ...
    /// <returns></returns>
    @Select({
            "call VideoClassMain_SP_Subject_UpdateFilePathV2(",
            "#{model.subjectId,mode=IN,jdbcType=INTEGER},",
            "#{model.filePath,mode=IN,jdbcType=DATE},",
            "#{model.realBeginTimeZone,mode=IN,jdbcType=INTEGER},",
            "#{model.classStateId,mode=IN,jdbcType=INTEGER})"
    })
    @Options(statementType = StatementType.CALLABLE)
    ...
    /// <returns></returns>
    @Select({
            "call VideoClassMain_SP_Subject_UpdatePPTFilePathV2(",
            "#{model.subjectId,mode=IN,jdbcType=INTEGER},",
    ...
    /// <param name="maxDateTime">最晚上课时间，将在此时间之前的上课中课程改为已下课</param>
    /// <returns></returns>
    @Update("update Subject set ClassStateID = #{classStateId} , PPTFilePath= #{pptFilePath}, RealBeginTimeZone = #{realBeginTimeZone}, BeginTime = now(), UpdateDateTime = now() where subjectId = #{subjectId}")
    void attendSubject(@Param("subjectId") int subjectId, @Param("pptFilePath") String pptFilePath, @Param("realBeginTimeZone") long realBeginTimeZone, @Param("classStateId") int classStateId);
    ...
    /// <param name="maxDateTime">最晚上课时间，将在此时间之前的上课中课程改为已下课</param>
    /// <returns></returns>
    @Update("update Subject set ClassStateID = #{classStateId} , PPTFilePath = #{pptFilePath}, EndTime = now(), UpdateDateTime = now() where subjectId = #{subjectId}")
    void closeSubject(@Param("subjectId") int subjectId, @Param("pptFilePath") String pptFilePath, @Param("classStateId") int classStateId);
