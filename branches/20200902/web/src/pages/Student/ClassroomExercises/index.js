...
export default class ClassroomExercises extends Component {
    state = {

    }
    componentDidMount() {
        const { dispatch, location: { query: { ExamSetId = '' } } } = this.props;
        dispatch({
    ...
    //提交接口方法
    submitFn = () => {
        const { dispatch, studentBasePath, location: { query: { ExamSetId = '' } } } = this.props;
        ...
        dispatch({
            ...
            if (res === 1) {
                const { location: { query: { ExamSetId = '' } } } = this.props;
                message.success('提交成功', 1.5)
                    .then(() => {
                        router.push(`${studentBasePath}/CourseManager/ViewAssessment?ExamSetId=${ExamSetId}`);
                    })
            } else {
