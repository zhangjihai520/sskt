...
import $ from 'jquery';
import style from './Listenclass.less';
...
import WorkIcon from 'assets/common/work_icon.png'
...
export default class ListenStream extends Component {
  ...
  handleRepeat = key => {
    const newPanes = [...this.state.panes];
    const nowPane = this.state.panes.find(pane => pane.key == key);
    var paneIndex = this.state.panes.findIndex(pane => pane.key == key);
    newPanes.splice(paneIndex, 1, { title: nowPane.title,messages: nowPane.messages.slice(nowPane.messages.length-50,nowPane.messages.length+1), key: nowPane.key, closable: nowPane.closable, scrollActive: nowPane.scrollActive});
    this.setState({ panes: newPanes});
  };
  ...
  handleGoExcise = () => {
    ...
    if (data.ExamSetId) {
      if (data.IsFinish) {
        ...
        window.open(`${studentBasePath}/CourseManager/ClassroomExercises?ExamSetId=${data.ExamSetId}`, '_blank');
      }
    } else {
      message.destroy();
  ...
  render() {
    const { data, message, tarVideoStream, UserStatus, userInfoList, emojiData, panes, activeKey, unReadKey} = this.state;
      ...
      <div className={style.contentArea}>
        ...
        <Row className={style.videoPanel}>
          <div className={style.leftPanel}>
              <div className={style.exciseBtn} onClick={this.handleGoExcise}>
                <img src={WorkIcon} title="练习" />
              </div>
          </div>
