...
import { Button, Checkbox, Input, message, Tabs, Popover, Collapse, Row, Col, Badge,Switch } from 'antd';
import $ from 'jquery';
...
export default class ListenStream extends Component {
  ...
  initWebSocket = () => {
    ...
    if (typeof (ws) == "undefined") {
      ...
      ws.onmessage = (evt) => {
        ...
        if (obj.TalkMessage && obj.MessageType == 0) {
          ...
          if(nowPane)
          {
            ...
            newPanes.splice(paneIndex, 1, { title: nowPane.title,messages: [...nowPane.messages,obj.TalkMessage], key: nowPane.key, closable: nowPane.closable, scrollActive: nowPane.scrollActive});
        ...
        this.setState(
          {
            UserId: result.Uid,
            wsUrl: url
          }, () => {this.getMessage(0);this.initWebSocket();});
  ...
  render() {
    const { data, message, tarVideoStream, noTalkingChecked, userInfoList, emojiData, panes, activeKey, unReadKey} = this.state;
    ...
    return (
      <div className={style.contentArea}>
        ...
        <Row className={style.videoPanel}>
          ...
          <Col flex="300px" className={style.video_right}>
            <Tabs defaultActiveKey="1" className={style.tabs}>
              <TabPane tab="用户列表" key="1">
                <div>
                  <Collapse defaultActiveKey={['1']} style={{ width: 300, height: '653px', overflowY: 'auto' }}>
                    <Panel key="1" header={
                      <span>
